...
class SubscriptionTransaction < Transaction
  ...
  def self.new_capture_payment(transacting_subscription, total_cost, tax_amount, at = Time.zone.now)
    transaction = SubscriptionTransaction.new()
    transaction.new_transaction_ledgers( transacting_subscription, TransactionAccount::CASH_ID, TransactionAccount::ACCOUNTS_RECEIVABLE_ID, total_cost, tax_amount, at)
    transaction
  end

  def self.new_cancel_captured_payment(transacting_subscription, total_cost, tax_amount, at = Time.zone.now)
    transaction = SubscriptionTransaction.new()
    transaction.new_transaction_ledgers( transacting_subscription, TransactionAccount::REVENUE_ID, TransactionAccount::CASH_ID, total_cost, tax_amount, at)
    transaction
  end
end
